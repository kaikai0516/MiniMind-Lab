...
def grpo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model, start_step=0, wandb=None, use_sglang=False):
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        model_unwrapped = get_raw_model(model)
        with autocast_ctx:
        ...
        if step % args.log_interval == 0 or step == iters:
            policy_loss_val = acc.loss_value
...
if __name__ == "__main__":
    ...
    # ========== 7. 编译和分布式包装 ==========
    model = wrap_model(model, bool(args.use_compile), local_rank, rollout_engine)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        grpo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model, start_step, wandb, use_sglang=(args.rollout_engine == "sglang"))
