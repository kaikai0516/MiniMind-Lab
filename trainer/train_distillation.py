...
def train_epoch(epoch, loader, iters, teacher_model, lm_config_student, start_step=0, wandb=None, alpha=0.0, temperature=1.0):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            model.eval()
            save_model_weights(model, args.save_dir, args.save_weight, lm_config_student.hidden_size, lm_config_student.use_moe)
            lm_checkpoint(lm_config_student, weight=args.save_weight, model=model, optimizer=optimizer, epoch=epoch, step=step, wandb=wandb, save_dir='../checkpoints', scheduler=scheduler)
            model.train()
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_epoch(epoch, loader, iters, teacher_model, lm_config_student, start_step, wandb, args.alpha, args.temperature)
