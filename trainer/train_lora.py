...
def train_epoch(epoch, loader, iters, lora_params, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            lm_checkpoint(lm_config, weight=args.lora_name, model=model, optimizer=optimizer, epoch=epoch, step=step, wandb=wandb, save_dir='../checkpoints', scheduler=scheduler)
            model.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, nargs='+', default=["../dataset/lora_medical.jsonl"], help="LoRA训练数据路径（支持多个文件/目录）")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_epoch(epoch, loader, iters, lora_params, start_step, wandb)
