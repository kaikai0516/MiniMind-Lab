...
def train_epoch(epoch, loader, iters, ref_model, lm_config, start_step=0, wandb=None, beta=0.1):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            lm_checkpoint(lm_config, weight=args.save_weight, model=model, optimizer=optimizer, epoch=epoch, step=step, wandb=wandb, save_dir='../checkpoints', scheduler=scheduler)
            model.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, nargs='+', default=["../dataset/dpo.jsonl"], help="DPO训练数据路径（支持多个文件/目录）")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_epoch(epoch, loader, iters, ref_model, lm_config, start_step, wandb, args.beta)
